    ...
    try {
      ...
      await logAudit(
        ...
        personId,
        {
          ...
          reason: reason.trim(),
          previousStatus: person.profileStatus,
        },
        req.ip
      );
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (!validRoles.includes(role)) {
        return res
          .status(400)
          .json({ error: "Invalid role. Must be GUEST, MEMBER, or ADMIN" });
      }
      ...
      await logAudit(
        ...
        userId,
        undefined,
        { action: "updated_user_role", newRole: role },
        req.ip
      );
...
const calculateGeneration = async (
...
const getPersonGeneration = async (personId: string): Promise<number> => {
  ...
  return await calculateGeneration(
    person.biologicalFatherId || undefined,
    person.biologicalMotherId || undefined
  );
};

// Get generation suggestion for new person
router.get(
  "/register/generation-suggestion",
  authenticate,
  requireAdmin,
  async (req: AuthRequest, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to calculate generation suggestion" });
    }
  }
);
