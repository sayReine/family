...
type TFn = (key: Parameters<ReturnType<typeof useLang>['t']>[0]) => string;

const navItems = (t: TFn, isAdmin: boolean) => [
    ...
    <>
      ...
      >
        ...
        <nav style={{ flex: 1, padding: '10px 10px', overflowY: 'auto' }}>
          {navItems(t as TFn, user?.role === 'ADMIN').map(({ to, icon, label, num }) => (
            <NavLink
              key={to}
              to={to}
              end={to === '/'}
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <div style={{ position: 'relative', flex: 1 }} ref={dropRef}>
              <button onClick={() => setLangOpen((o) => !o)} style={{ ...controlBtn, justifyContent: 'center' }}>
                <Globe size={13} />
                <span style={{ textTransform: 'uppercase' }}>{lang}</span>
                <ChevronDown
                  size={11}
                  style={{ transition: 'transform 0.15s', transform: langOpen ? 'rotate(180deg)' : 'none' }}
                />
              </button>
