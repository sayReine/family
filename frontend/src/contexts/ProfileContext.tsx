...
export const ProfileProvider: React.FC<{ children: ReactNode }> = ({
  ...
  // Get auth context
  const { token, register } = useBackendAuth();
  ...
  const buildPayload = (status: "DRAFT" | "PENDING") => {
    return {
      ...
      dateOfDeath: profileData.isDeceased
        ? profileData.dateOfDeath || null
        : null,
  ...
  const saveDraft = async () => {
    ...
    if (!token) {
      throw new Error(
        "You must be logged in to save a draft. Please register or login first."
      );
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || `Failed to save draft: ${response.status}`
        );
      }
  ...
  const submitProfile = async () => {
    ...
    if (!token) {
      throw new Error(
        "You must be logged in to submit a profile. Please register or login first."
      );
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || `Failed to submit profile: ${response.status}`
        );
      }
  ...
  const registerAndSubmit = async () => {
    ...
    try {
      ...
      console.log(
        "Step 2: User registered successfully, now submitting profile"
      );
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Update the local state with the loaded profile data
        setProfileData({
          firstName: profileData.firstName || "",
          ...
          isDeceased: profileData.isDeceased || false,
          dateOfDeath: profileData.dateOfDeath || "",
          ...
          biologicalFatherId: profileData.biologicalFatherId || undefined,
          biologicalMotherId: profileData.biologicalMotherId || undefined,
          spouses: profileData.spouses || [],
          childrenIds: profileData.childrenIds || [],
          bio: profileData.bio || "",
        ...
        console.log("No existing profile found, keeping initial state");
        // No existing profile, keep the initial state
      } else {
        const errorText = await response.text();
        console.error("Error loading profile:", errorText);
        // Don't throw error for profile loading, just log it
      }
    } catch (error) {
      console.error("Error loading profile:", error);
      // Don't throw error for profile loading
    } finally {
      setIsLoading(false);
    }
  };
